...
现已补充互动写入预算、积压保护和鉴权故障超时，并完成 20 RPS 的 30 分钟长测及依赖故障验证，修复后另做五分钟复测。版本边界、失败记录和完整数据见 [工程验收报告](docs/capacity/2026-09-26-steady/report.md)。默认写入预算为每 API 进程 20 RPS，不代表生产或多副本容量。
